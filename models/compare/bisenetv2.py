...
class DetailBranch(nn.Module):
    def __init__(self):
        ...
        self.S1 = nn.Sequential(
            ConvBNReLU(3, 64, 3, stride=2),#下采样1/2
            ConvBNReLU(64, 64, 3, stride=1),
        )
        self.S2 = nn.Sequential(
            ConvBNReLU(64, 64, 3, stride=2),#下采样1/4
            ConvBNReLU(64, 64, 3, stride=1),
            ConvBNReLU(64, 64, 3, stride=1),
        )
        self.S3 = nn.Sequential(
            ConvBNReLU(64, 128, 3, stride=2),#下采样1/8
...
class SegmentBranch(nn.Module):#语义分支
...
class BiSeNetV2(nn.Module):
    def __init__(self, num_classes=19, augment=True):
        ...
        self.head = SegmentHead(128, 1024, num_classes, up_factor=8, aux=False)
        # self.aux2 = SegmentHead(16, 128, num_classes, up_factor=4)
        self.aux3 = SegmentHead(32, 128, num_classes, up_factor=8)
        # self.aux4 = SegmentHead(64, 128, num_classes, up_factor=16)
        # self.aux5_4 = SegmentHead(128, 128, num_classes, up_factor=32)
    ...
    def forward(self, x):
        ...
        aux, feat_s = self.segment(x)
        feat_head = self.bga(feat_d, feat_s)

        logits = self.head(feat_head)

        if self.augment:
            logits_aux3 = self.aux3(aux)
            return [logits_aux3, logits]
        else:
            return logits
